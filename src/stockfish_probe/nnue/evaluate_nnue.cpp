...
static bool read_parameters(std::istream &stream, NetSize netSize,
  ...
  if (hashValue != HashValue[netSize])
    return false;
  if (netSize == Big &&
      !Detail::read_parameters(stream, *featureTransformerBig))
    return false;
  if (netSize == Small &&
  ...
  for (std::size_t i = 0; i < LayerStacks; ++i) {
    if (netSize == Big && !Detail::read_parameters(stream, *(networkBig[i])))
      return false;
    if (netSize == Small &&
        !Detail::read_parameters(stream, *(networkSmall[i])))
      return false;
  }
...
void verify_accumulators(const Position &pos) {
  featureTransformerBig->hint_common_access(pos);
  featureTransformerSmall->hint_common_access(pos);
}
...
Value evaluate(const Position &pos, bool adjusted, int *complexity) {
  ...
  TransformedFeatureType transformedFeaturesUnaligned
      [FeatureTransformer < Net_Size == Small
...
bool save_eval(
  ...
  if (filename.has_value())
    ...
    if (evalFiles.at(netSize).current != (netSize == Small
