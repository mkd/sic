...
template <PieceType PT> constexpr auto make_piece_indices_type() {
...
template <Piece P> constexpr auto make_piece_indices_piece() {
...
constexpr auto index_lut2_array() {
  ...
  std::array<std::array<std::array<uint8_t, SQUARE_NB>, SQUARE_NB>, PIECE_NB>
      indices{};
  ...
  for (Piece piece : AllPieces) {
    ...
    for (Square from = SQ_A1; from <= SQ_H8; ++from) {
      ...
      if (type_of(piece) != PAWN) {
        ...
        Bitboard attacks = (pieceIdx < 8) ? PawnPushOrAttacks[WHITE][from]
                                          : PawnPushOrAttacks[BLACK][from];
        cumulativePieceOffset += constexpr_popcount(attacks);
      }
    }
...
constexpr auto init_index_luts() {
  ...
  for (Piece attacker : AllPieces) {
    for (Piece attacked : AllPieces) {
      ...
      bool excluded = map < 0;
      indices[attacker][attacked][0] =
          excluded ? FullThreats::Dimensions : feature;
...
// The final index is calculated from summing data found in these two LUTs, as
// well as offsets[attacker][from]
...
FullThreats::make_index(Color perspective, Piece attacker, Square from,
  ...
  return index_lut1[attacker_oriented][attacked_oriented]
                   [from_oriented < to_oriented] +
...
void FullThreats::append_active_indices(Color perspective, const Position &pos,
                                        IndexList &active) {
  ...
  for (Color color : {WHITE, BLACK}) {
...

    {
      ...
      // Set of pawns which are prevented from movement by a pawn in front of
      // them
...
void FullThreats::append_changed_indices(Color perspective, Square ksq,
                                         ...
                                         FusedUpdateData *fusedData, bool first,
  ...
  for (const auto &dirty : diff.list) {
    ...
    if (prefetchBase)
      prefetch((void *)(reinterpret_cast<uintptr_t>(prefetchBase) +
                        index * prefetchStride));
